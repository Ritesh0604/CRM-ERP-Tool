...
export default function UpdatePayment({ config, currentInvoice }) {
		...
		if (currentInvoice) {
			...
			setMaxAmount(
				calculate.sub(
					...
					calculate.sub(calculate.sub(credit, amount)),
				),
			);
	...
	}, [isSuccess, currentInvoice, navigate, entity, dispatch, form]);
	...
	const onSubmit = (fieldsValue) => {
		...
		if (currentInvoice) {
			...
			const client = currentInvoice.client?._id;
			updatedFieldsValue = {
				...updatedFieldsValue,
				invoice,
				client,

...
function FormElement({ field, feedback, setFeedback }) {
	...
	const SelectWithFeedbackComponent = ({ feedbackValue, lachFeedback }) => (
		<Form.Item
			label={translate(field.label)}
			name={field.name}
			rules={[
		...
		>
			<Select
				onSelect={(value) => lachFeedback(value)}
				value={feedbackValue}
				style={{
					width: "100%",
				}}
	...
	let renderComponent = communedComponent[field.type];

	if (!renderComponent) {
		renderComponent = communedComponent.string;
	}

...
const MultiStepSelectAsync = ({
	...
	useEffect(() => {
		...
		fetchData();
	}, [firstSelectedOption, entityName, subEntityName, firstSelectIdKey]);
		...
		<Space direction="vertical" style={style}>
			...
			>
				{firstSelectOptions.map((option) => (
					<Option
						key={option[firstSelectIdKey]}
						value={option[firstSelectValueKey]}
					>
			...
			{firstSelectedOption && (
				...
				>
					{secondSelectOptions.map((option) => (
						<Option
							key={option[secondSelectIdKey]}
							value={option[secondSelectValueKey]}
						>
